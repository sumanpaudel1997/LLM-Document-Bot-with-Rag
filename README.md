...
- **Using Python**:
    ...
    - 4. If you don't have virtual environment install using `pip install virtualenv`
    - 5. Activate virtual environment `source venv/bin/activate`
    - 6. Install the required dependencies using `pip install -r requirements.txt`
    - 7. After successful installation run the application with `python main.py`

- **Using Docker**:
    - 1. create a `.env` file and add your api_key OPENAI_API_KEY='yoursecretkey' or do `export OPENAI_API_KEY=<yourkey>`
...
- Use the app to query the pdf file.
- The app is purely CLI based where users are prompted to ask the questions of their files.
...
- **qdrant**: A folder containing qdrant local instance to save the vector embeddings.
...
- **storage_index**: Folder containing files that store storage context container which is a utility container for storing nodes, indices, and vectors.
- `requirement.txt` : all of the dependencies needed to run the project.
- `Dockerfile` : Contains dockerized version of the application
- `.gitignore` : Contains a list of the files that are ignored while pushing. e.g .env file, log files 
...
- **Vector DB**
    ...
    - After installing weaviate client. Create connection to weaviate client on the local instance using 
    ...
    - You can also use the cloud version of Weaviate. See cloud version [here](https://weaviate.io/developers/weaviate/installation/weaviate-cloud-services)

- **Embedding Model**
    - The default system embedding model is HuggingFaceEmbedding: `model_name="BAAI/bge-small-en-v1.5`
    ...
    - You can also use an alternate model. Here is a sample for OpenAI.
        - Install the model first using `pip install llama-index-embeddings-openai` `pip install -U openai`
        - Change the following command in the `main.py` file where the huggingface model is located in the code.
